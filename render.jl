...
function load_matches(mesh_set, idx)
...
function warp_tile(mesh_set, tile_path, idx)
...
#   img_warped: tile image warped by piecewise affine solution of mesh
    ...
    src_pts, dst_pts = load_matches(mesh_set, idx)
...
# Load JLD file for MeshSet, warp images, then display
    ...
    A, SR_A = warp_tile(mesh_set, tile_pathA, 1) 
    
    # r4_c3
    tile_pathB = joinpath(BUCKET, "EM_images", "Tile_r4-c3_S2-W001_sec20.tif")
    B, SR_B = warp_tile(mesh_set, tile_pathB, 2)  

    src_pts, dst_pts = load_matches(mesh_set, 1)
    draw_points(make_isotropic(rawdata(imread(tile_pathA))), src_pts)
    draw_points(make_isotropic(rawdata(imread(tile_pathB))), dst_pts)
