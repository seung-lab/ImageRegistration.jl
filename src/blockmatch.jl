...
`matches = blockmatch(nodes, src_img, dst_img, src_offset, dst_offset, params)`
...
function blockmatch(nodes, src_img, dst_img, src_offset, dst_offset, params)
    n = size(nodes, 1)
    displacements = Array{Tuple{Array{Float64,2},Float64},1}(undef, n)
    src_ranges = Array{Tuple{UnitRange{Int64},UnitRange{Int64}},1}(undef, n)
    dst_ranges = Array{Tuple{UnitRange{Int64},UnitRange{Int64}},1}(undef, n)
