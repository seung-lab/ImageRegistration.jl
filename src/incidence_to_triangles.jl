...
"""
`INCIDENCE_TO_INCIDENT_LIST_ARRAY` - takes in M edges on N nodes in NxM sparse
 * format and returns an 3xN array where the N-th column includes the indices
 * of the nodes connected (one-direction) to node N
...
function incidence_to_incident_list_array(E)
  ...
  # for each edge
    for j = 1:size(E, 2)
        ...
        incident_list_array[incident_list_counts[node_first], node_first] = node_second
        incident_list_counts[node_first] += 1
        ind = ind + 2
    end
...
function incident_list_array_to_triangles(incident_list_array)
  ...
  # for each node at j
    for p = 1:size(incident_list_array, 2)
	...
	# the up to 9 points that have edges from pa, pb, pc
            paa = incident_list_array[1, pa]
            pab = incident_list_array[2, pa]
            pac = incident_list_array[3, pa]
        else
            paa = 0
            pab = 0
            pac = 0
        ...
        if pb != 0
            pba = incident_list_array[1, pb]
            pbb = incident_list_array[2, pb]
            pbc = incident_list_array[3, pb]
        else
            pba = 0
            pbb = 0
            pbc = 0
        ...
        if pc != 0
            pca = incident_list_array[1, pc]
            pcb = incident_list_array[2, pc]
            pcc = incident_list_array[3, pc]
        else
            pca = 0
            pcb = 0
            pcc = 0
        ...
        cur_triangle = check_and_write_triangles!(
            triangles,
            cur_triangle,
            p,
            pa,
