...
function meshwarp(
    ...
    Us = Array{Array{Float64,1}}(undef, size(trigs, 2))
    Vs = Array{Array{Float64,1}}(undef, size(trigs, 2))
    Ms = Array{Array{Float64,2}}(undef, size(trigs, 2))
    ...
    @fastmath @inbounds for t = 1:size(trigs, 2)
  ...
  #=@sync @fastmath @inbounds for t in 1:size(trigs, 1);
    ...
    @sync for p in procs()
        t = proc_range(p, Us)
        @async @fastmath @inbounds remotecall_wait(
            calculate_pixels_in_trig_chunk!,
            p,
            Us[t],
            Vs[t],
            Ms[t],
            ...
            warped_img,
            warped_offset,
            interp ? computepixel_interp : computepixel
        )
    end
...
function calculate_pixels_in_trig_chunk!(
    Us,
    Vs,
    Ms,
    img,
    ...
    @simd for i = 1:length(Us)
        calculate_pixels_in_trig!(
            Us[i],
            Vs[i],
            Ms[i],
            img,
...
function calculate_pixels_in_trig!(
    U,
    V,
    M,
    img,
    ...
    for j_poly = 1:size(poly, 2)
        for i_poly = 1:size(poly, 1)
            @fastmath @inbounds begin
	#check if poly is true
                if poly[i_poly, j_poly] == false
                    continue
                end
        ...
        # x, y = M * [u, v, 1]
                @fastmath @inbounds x, y = M[1, 1] * u + M[2, 1] * v + M[3, 1],
                    M[1, 2] * u + M[2, 2] * v + M[3, 2]
...
@inline function poly2source_new!(pts_i, pts_j)
  # Find bb of vertices (vertices in global space)
    ...
    left, right = floor(Int64, minimum(pts_j)), ceil(Int64, maximum(pts_j))
  # Create image based on number of pixels in bb that will identify triangle
    if size(MESHWARP_POLY2SOURCE_MASK::Array{Bool,2})[1] < bottom - top + 1 ||
       size(MESHWARP_POLY2SOURCE_MASK)[2] < right - left + 1
...
function fillpoly!(
    ...
    py::Vector{P},
    value::T;
    convex::Bool = false, reverse::Bool = false
) where {T,P<:Number}
    convex ? (return fillpoly_convex!(M, px, py, value; reverse = reverse)) :
    (return fillpoly_nonconvex!(M, px, py, value; reverse = reverse))
end
...
function fillpoly_convex!(
  ...
  # Scan poly from left to right
    for x in xrange     # loop over grid lines
        top = 0
        bot = 0
        m = length(px)
...
#	  end
    ...
    # Place value in matrix at all the y's between min and max for given x
        for n = 1:2:length(ys_clean)
            @simd for y = ys_clean[n]:ys_clean[n+1]
                @inbounds M[y, x] = value
            end
        end
    end
