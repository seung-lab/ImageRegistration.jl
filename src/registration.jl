...
function default_params()
    return Dict("mesh_dist" => 750, "block_size" => 200, "search_r" => 150, "min_r" => 0.25)
end
...
function blockmatch(
    src_img,
    dst_img;
    src_offset = [0, 0], dst_offset = [0, 0], params = default_params()
)
    mesh = create_mesh(src_img, src_offset, params["mesh_dist"])
    matches = blockmatch(mesh.src_nodes, src_img, dst_img, src_offset, dst_offset, params)
    return mesh, matches
end
...
function matches_to_mesh(matches::Matches, mesh::Mesh)
    ...
    non_orphans = find_zero_indices(sum(edges, dims = 1))
